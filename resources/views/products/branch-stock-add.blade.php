...
@section('title', 'Tambah Stok Cabin - ' . $product->name)

@section('content')
<div class="max-w-2xl mx-auto">
    <div class="mb-6">
        <div class="flex items-center gap-2 mb-2">
            <a href="{{ route('products.branch-stock', $product) }}" class="text-gray-500 hover:text-gray-700">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
                </svg>
            </a>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Tambah Stok Cabin - {{ $product->name }}</h1>
        </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        <form action="{{ route('products.branch-stock.store', $product) }}" method="POST">
            @csrf
            ...
            <div class="mb-6 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                ...
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Nama produk: <strong>{{ $product->name }}</strong></p>
            </div>
            ...
            <div class="mb-6">
                ...
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Contoh: jika input 10, stok di cabin tersebut bertambah 10 unit.</p>
            </div>

            <div class="flex justify-end gap-3">
                <a href="{{ route('products.branch-stock', $product) }}" class="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700">
                    Batal
                </a>
...
<script>
const stockData = <?= json_encode($product->branchStocks()->pluck('stock', 'branch_id')->toArray()) ?>;
...
function updateCurrentStock(branchId) {
    ...
    if (branchId && stockData[branchId] !== undefined) {
        display.textContent = stockData[branchId] + ' unit';
    } else {
